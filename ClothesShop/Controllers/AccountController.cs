...
namespace ClothesShop.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = await UserManager.FindByNameAsync(model.UserName);//ByUserName
            if (user != null)
            {
                if (!await UserManager.IsEmailConfirmedAsync(user.Id))
                {
                    ViewBag.errorMessage = "Bạn phải xác minh tài khoản trước khi đăng nhập.";
                    return View("Error");
                }
            }
        ...
        public async Task<ActionResult> Register(CreateAccountViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(user.Id, "Xác minh tài khoản", "Xác minh tài khoản của bạn bằng cách truy cập link sau <a href=\"" + callbackUrl + "\">here</a>");
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = await UserManager.FindByEmailAsync(model.Email);//byEmail
                ...
                await UserManager.SendEmailAsync(user.Id, "Tạo lại mật khẩu", "Tạo lại mật khẩu bằng cách truy cập link sau <a href=\"" + callbackUrl + "\">here</a>");
                //return RedirectToAction("ForgotPasswordConfirmation", "Account");
                ViewBag.Message = "Kiểm tra Email.";
                return View("Info");
